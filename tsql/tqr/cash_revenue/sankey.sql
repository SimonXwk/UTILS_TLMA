...
cet_mysourcecode AS (
  SELECT SOURCECODE,SOURCETYPE
    ...
    , ADDITIONALCODE3  AS [CAMPAIGN]
  	...
  	,IIF(RTRIM(ISNULL(ADDITIONALCODE3,'')) = '' OR LEFT(UPPER(LTRIM(ADDITIONALCODE3)),4) = 'TLMA', @UNSOLICITED, @SOLICITED) AS [SOLICIT]
...
,cte_sankey_attributes AS (
  SELECT
    ...
    , S1.PLATFORM
    , S1.TQTYPE
    , IIF(RTRIM(ISNULL(S1.CAMPAIGN,'')) = '' OR LEFT(UPPER(LTRIM(S1.CAMPAIGN)),4) = 'TLMA', @UNSOLICITED, S1.CAMPAIGN) AS [CAMPAIGNCODE]
  ...
  WHERE
    ...
    AND B2.DATEOFPAYMENT BETWEEN '20180701' AND '20190630'
  GROUP BY
    S1.SOLICIT, S1.PLATFORM, S1.TQTYPE, S1.ACCOUNT, S1.CAMPAIGN, S1.CLASS
    , B1.DESTINATIONCODE
    , B1.DESTINATIONCODE2
    , B1.SOURCECODE
    , B1.SOURCECODE2
