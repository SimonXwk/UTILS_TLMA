...
cte_payments as (
  SELECT
    ...
    , [ISMERCHGOL]=IIF(S1.SOURCETYPE LIKE 'Merch%Gift%of%love%', -1, 0)
...
,cte_rex_orders as (
  SELECT SERIALNUMBER, DATEOFPAYMENT,
    CASE WHEN NOT MANUALRECEIPTNO LIKE @REX_RECEIPT_PATTERN
    THEN
      SUBSTRING(NOTES
        ,PATINDEX(@REX_RECEIPT_NOTE_PATTERN,NOTES) + LEN(@REX_RECEIPT_PREFIX) + 1
        ,11)
    ELSE LTRIM(RTRIM(MANUALRECEIPTNO))
    END AS [ORDERID]
  FROM TBL_BATCHITEM
  WHERE
...
where
  t1.FIRSTDATE between '20170701' and '20180630'
group by
  t1.SERIALNUMBER,t1.FIRSTDATE
  ,t3.CONTACTTYPE, t3.PRIMARYCATEGORY, t3.DONOTMAIL, t3.DONOTMAILREASON, t3.DONOTCALL, t3.SORTKEYREF1, t3.SORTKEYREFREL1 ,t3.SORTKEYREFREL2
having
